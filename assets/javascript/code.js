...
$(document).ready(function () {
	...
	function updateStats() {
		...
		if (!finishGame) {
			...
			if (mode === "challenge") {
				...
				if (level === 10) {
					// All levels completed on CHALLENGE mode.
					// console.log("HERE");

					// Set the massage
					msg = $("<div>")
						.html("<h5>YOU FOUND ALL<br>THE MATCHES ON<br>ALL LEVELS!</h5>")
						.appendTo($("#updateText"));
				} else if (level < 9) {
					// Game done for CHALLENGE modes:

					// Set the massage
					msg = $("<div>")
						.html(
							"<h5>YOU FINISHED LEVEL " + level + "<br>GO TO THE NEXT ONE</h5>"
						)
						.appendTo($("#updateText"));
				...
				// Game done for EASY and TIMED modes
			}
	...
	function getFlagURL() {
		///////////////////// restcountries.eu ///////////////////////
		var queryURL = "https://restcountries.com/v3.1/name/" + userCountry;
		...
		$.ajax({
			...
			userCountryFlag = response[0].flags.svg;
	...
	$(".btnPlay").on("click", function (e) {
		...
		if ($("#nameInput").val() === "" || $("#countryInput").val() === "") {
			...
			$("#errorText").text(
				"YOU NEED TO ENTER A USERNAME AND A SELECT A VALID COUNTRY"
			);
	...
	function decrement() {
		...
		if (time <= 0) {
			...
			// Add the clock image
			var img = $("<img>")
				.attr("src", "assets/images/clock.png")
				.attr("id", "updateImage")
				.appendTo($("#updateText"));

			// Set the massage
			var msg = $("<div>")
				.html("<h5>YOUR TIME<br>IS UP!</h5>")
				.appendTo($("#updateText"));
